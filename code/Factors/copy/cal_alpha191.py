...
benchmark_price = pd.read_excel('nh0100.xlsx').set_index('date')
...
class CalAlpha:
    ...
    def alpha_047(self):
        ...
        result=pd.ewm(100*part1/part2,alpha=1.0/9)
        alpha=result.iloc[-1,:].dropna()
        return alpha   
    ...
    def alpha_057(self):
        ...
        result=pd.ewm(100*part1/part2,alpha=1.0/3)
        alpha=result.iloc[-1,:]
        return alpha.dropna()
    ...
    def alpha_063(self):
        ...
        part1=pd.ewm(part1,alpha=1.0/6)
        part2=(self.close-self.close.shift()).abs()
        part2=pd.ewm(part2,alpha=1.0/6)
    ...
    def alpha_067(self):
        ...
        part1=pd.ewm(part1,alpha=1.0/24)
        temp2=temp1.abs()
        part2=pd.ewm(temp2,alpha=1.0/24)
        result=part1*100/part2
        alpha=result.iloc[-1,:].dropna()
        return alpha
    ...
    def alpha_068(self):
        ...
        result=pd.ewm(result,alpha=2.0/15)
        alpha=result.iloc[-1,:].dropna()
        return alpha
    ...
    #     
        ...
        alpha = pd.ewm(data1 / data2 * 100, alpha=1/15).iloc[-1].dropna()
        return alpha
    ...
    #     
        condition = benchmark_price['close'] < benchmark_price['open']
        data1 = benchmark_price[condition]
        numbench = len(data1)
    ...
    #     
        data1 = pd.ewm(np.maximum((self.close - self.prev_close), 0), alpha=1/12)
        data2 = pd.ewm(abs(self.close - self.prev_close), alpha=1/12)
    ...
    #############################################################################
    def alpha_081(self):
        result=pd.ewm(self.volume,alpha=2.0/21)
        alpha=result.iloc[-1,:].dropna()
        return alpha
    ...
    def alpha_082(self):
        part1=pd.rolling_max(self.high,6)-self.close
        part2=pd.rolling_max(self.high,6)-pd.rolling_min(self.low,6)
        result=pd.ewm(100*part1/part2,alpha=1.0/20)
        alpha=result.iloc[-1,:].dropna()
        return alpha
    ...
    def alpha_89(self):
        ...
        ######################      
        data1=pd.ewm(self.close,span=12,adjust=False)
        data2=pd.ewm(self.close,span=26,adjust=False)
        data3=pd.ewm(data1-data2,span=9,adjust=False)
    ...
    def alpha_96(self):
        ...
        sma1 = pd.ewm(100*(self.close-self.low.rolling(9).min())/(self.high.rolling(9).max()-self.low.rolling(9).min()), span=5, adjust=False)
        alpha = pd.ewm(sma1, span=5, adjust=False).iloc[-1,:]
        alpha=alpha.dropna()
        return alpha
    ...
    def alpha_102(self):
        ...
        max_data[~max_cond] = 0
        sma1 = pd.ewm(max_data, span=11, adjust=False)
        sma2 = pd.ewm((self.volume-self.volume.shift()).abs(), span=11, adjust=False)
    ...
    def alpha_109(self):
        ...
        sma1 = pd.ewm(data, span=9, adjust=False)
        sma2 = pd.ewm(sma1, span=9, adjust=False)
    ...
    def alpha_111(self):
        ...
        x=pd.ewm(data1,span=10)
        y=pd.ewm(data1,span=3)
        alpha=(x-y).iloc[-1,:]
        alpha=alpha.dropna()
        return alpha
    ...
    def alpha_122(self):
        ...
        data = pd.ewm(pd.ewm(pd.ewm(log_close, span=12, adjust=False), span=12, adjust=False), span=12, adjust=False)
    ...
    def alpha_135(self):
        ...
        alpha = pd.ewm(data1, com=19, adjust=False).iloc[-1,:]
        alpha=alpha.dropna()
        return alpha 
    ...
    def alpha_152(self):
        ...
        #       
        data1 = pd.rolling_mean((pd.ewm(((self.close/self.close.shift(9)).shift()), span=17, adjust=False)).shift(), 12)
        data2 = pd.rolling_mean((pd.ewm(((self.close/self.close.shift(9)).shift()), span=17, adjust=False)).shift(), 26)
        alpha = (pd.ewm(data1-data2, span=17, adjust=False)).iloc[-1,:]
        alpha=alpha.dropna()
        return alpha
    ...
    def alpha_155(self):
        ...
        sma1 = pd.ewm(self.volume, span=12, adjust=False)
        sma2 = pd.ewm(self.volume, span=26, adjust=False)
        sma = pd.ewm(sma1-sma2, span=9, adjust=False)
        alpha = sma.iloc[-1,:]
        alpha=alpha.dropna()
        return alpha
    ...
    def alpha_158(self):
        ...
        alpha = (((self.high-pd.ewm(self.close, span=14, adjust=False))-(self.low-pd.ewm(self.close, span=14, adjust=False)))/self.close).iloc[-1,:]
        alpha=alpha.dropna()
        return alpha
    ...
    def alpha_164(self):
        ...
        alpha=(pd.ewm(data3,span=12)).iloc[-1,:]
        alpha=alpha.dropna()
        return alpha
    ...
    def alpha_169(self):
        ...
        data1=self.close-self.close.shift()
        data2=(pd.ewm(data1,span=17)).shift()
        data3=pd.rolling_mean(data2,12)-pd.rolling_mean(data2,26)
        alpha=(pd.ewm(data3,span=19)).iloc[-1,:]
        alpha=alpha.dropna()
        return alpha  
    ...
    def alpha_173(self):
        ...
        data1=pd.ewm(self.close,span=12)
        data2=pd.ewm(data1,span=12)
        close_log=np.log(self.close)
        data3=pd.ewm(close_log,span=12)
        data4=pd.ewm(data3,span=12)
        data5=pd.ewm(data4,span=12)
    ...
    def alpha_174(self):
        ...
        data2[~cond] = 0
        alpha=(pd.ewm(data2,span=39,adjust=False)).iloc[-1,:]
        alpha=alpha.dropna()
        return alpha
    ...
    def alpha_188(self):
        ...
        sma = pd.ewm(self.high - self.low, span=10, adjust=False)   
...
alpha_002_factor = pd.DataFrame(index = index_date,columns = columns_security)
...
# os.chdir(r'D:\Xin\Program\Alpha_Trading\data\raw_data\daily_data')
...
#     cal_alpha = CalAlpha(data)
#     alpha_names = [method for method in dir(cal_alpha) if callable(getattr(cal_alpha, method)) if not method.startswith('_')]
#     del (alpha_names[-5:])
#     num_alpha = len(alpha_names)
#     alpha_values = np.zeros(num_sample)

#     for j in range(num_alpha):
#         alpha_code = alpha_names[j]
#         alpha = getattr(cal_alpha, alpha_code)()
        # alpha_values[:, j] = alpha

#     alpha_df = pd.DataFrame(data=alpha_values, columns=alpha_names)
#     alpha_df = alpha_df.replace([np.inf, -np.inf, np.nan], 0)
#     data_with_alpha = pd.concat([data, alpha_df], axis=1)
#     data_with_alpha.to_excel(save_path + str(option_code) + '.xlsx', index=False)